...
#         self.conv2 = GCNConv(dim, dim*2)
#         self.conv3 = GCNConv(dim*2, dim * 4)
#         self.fc_g1 = torch.nn.Linear(dim*4, 1024)
...
# #         self.W_rnn = nn.GRU(bidirectional=True, num_layers=2, input_size=dim, hidden_size=dim, batch_first=True)
...
#         x = self.conv1(fingerprint_vectors, adjacency)
#         x = self.relu(x)
...
#         x = self.dropout(x)
#         x = self.fc_g2(x)
#         compound_vector = self.dropout(x)
...
#         after_smile_vectors = self.attention_cnn(smile_vectors, layer_cnn)
